let accessToken;
let articleId;
describe("Create article comment with status code 200", () => {
  before(() => {
  ...
  it("Checking if should be able to create article comment", () => {
    cy.request({
      method: "POST",
      ...
      body: { contentId: articleId, comment: "Anything" },
      failOnStatusCode: false,
    }).then((response) => {
      expect(response.status).to.eq(201, "Expected status code is 200");
      if (response.status === 201) {
        const { body, duration } = response;
        cy.writeFile("cypress/fixtures/commentId.json", {
